...
**Answer** So we don't overide our initial table of journalism classes that has eight columns.
...
**Answer:** The coulumn is now considered a date (<date>) rather than a string of characters (<chr>).
...
**Answer** Because date is formatted as year/month/day, the first day of the month is still associated with with the month itself. Therefore it's still showing the day even though the data is portraying money spent over the entire month.
...
**Answer** We are using existing data to create a new variable (total_votes) that we'll use to calculate percentages that represent the vote share of each candidate, starting with Wes Moore.
...
**Answer** The counties are now sorted greatest to least by the percentage of the vote Moore won.
...
**Answer** I'd say it's just as interesting, it tells us something different if you wanted to look at the counties where Moore won the least amount of the vote share, but that info is also already available in the other chart, just at the bottom.
...
**Answer** Other functions include "toupper(x)," which you need to have a string before in order for it to manipulate text, and "casefold(x, upper=true/false)," which you can use to change a string to all uppercase or lowercase.
...
**Answer** At each step, it is finding a state that's been requested to be detected and changing it to a specified abbreviation. At the end, I think it's setting any addresses not in those states to "NA."
...
**Answer** It makes sense to me because Maryland would be expected to have the largest amount of donation for a Maryland gubernatorial race. Also, NA being second makes sense because it's grouping the other 40 stats in the U.S. that were not specified in the code.
...
1. Which UMD course has the highest syllabus count?

2. Which UMD course has the lowest number of seats, not counting classes that have 0 seats associated with them?

3. What percentage of Wes Moore's committee's expenses used for campaign staff wages?
