...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** The are mostly the same, but the order of a couple columns are swapped over the course of the four tables.
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** The total number of voters has grown by about 35,000. It seems like during that time, the number of Democrats and unaffiliated voters are the groups that have primarily grown during that time.
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 rows in the table, which is exactly how many there should be.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** The information contains age and race data for each Maryland jurisdiction.
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's County has the highest 18+ Black population as a percentage of all population 18 and over with 60.9% — Prince George's County's population is 77.1% Democrats.
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Howard County has the highest 18+ Asian population as a percentage of all population 18 and over with 19.37% — Howard County's population is 52.57% Democrats.
