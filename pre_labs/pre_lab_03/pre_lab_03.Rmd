...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer**  No, there are no parsing issue like there was in the last question.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** The vendor name field is a character. The amount field is a double. The date column is a character.
...
The are 549 zip codes that are smaller than 5 digits from our data set, and a lot of them are repeated. I think this function might not count a leading 0 as a part of the string length, making it look like there is a bunch of incorrect data when really a lot of these zip codes actually are made up of 5 numbers.
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer** There are some categories labelled as NA with small amounts of data in them that aren't classified as a grant or a loan, meaning there is data that is labelled incorrectly. There is also a category labelled as just "L" in 2013.
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer** It seemed kind of weird that there were loans for such small amounts of money, including one for only $60, which was the smallest loan in the data set.
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer** It standardized the column names so they were lowercase and connected by a underscore if they were multiple words.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** There are 50 rows in the data set, which means there's a possible 25 duplicates in the data, which a reporter should double check to see if there's an error in the data.
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer** The new data frame has 17,740 rows — 25 less than the original data set did.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** There is a new column in which the zip codes with the four digit extensions were shortened so only the first five digits of a zip code is shown. 
...
At first the top zip code didn't make sense to me because it was in Washington, D.C., but I realized that those grants were probably for transportation projects managed by WMATA, which is located in the nation's capital but also services Maryland.
