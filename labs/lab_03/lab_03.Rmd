...
md_payments <- read_csv("data/State_of_Maryland_Payments_Data__FY2008_to_FY2024.csv", guess_max=322138) %>% clean_names
...
A1: I found 39 different records.

```{r}
pg_schools <- md_payments |>
  filter(vendor_name == "PRINCE GEORGES PUBLIC SCHOOLS" | vendor_name == "BOARD OF EDUCATION OF PRINCE GEORGES COUNTY MD" | vendor_name == "PRINCE GEORGES SCHOOL" | vendor_name == "PRINCE GEORGES COUNTY SCHOOL" | vendor_name == "BD OF EDUCATION OF PG CO" | vendor_name == "PRINCE GEORGES COUNTY PUBLIC SCHOOLS" | vendor_name == "BOARD OF EDUCATION OF PRINCE GEORGE'S COUNTY" | vendor_name == "PG COUNTY PUBLIC SCHOOL")
...
A2: The fiscal year 2020 had the largest total amount of payments to the Prince George's County School system with $1,362,372,202. The data spans from 2013 to 2020 but is missing the year 2016.
...
A3. Three vendor names that are similar that could be combined into one are A MORTON THOMAS & ASSOC INC and A MORTON THOMAS & ASSOCIATES INC, A&I INC and A&I INC. as well as AMTEK COMPANY INC and AMTEK COMPANY, INC. There is some cleaning that needs to be done on the data in order to prevent the many duplicates in the data, and some potential fixes could be deleting punctuation marks and taking common abbreviation like "Inc." and "Incorporation" into account.
...
A4: Stem cell research grants have declined year after year over the last decade or so since peaking in 2009.
