...
md_votes_county <- read_csv("data/md_statewide_county.csv") |> clean_names() |>
  mutate(pct_moore = moore / (moore + cox + lashar + wallace + harding + sefcik + gov_writein) * 100) |>
  mutate(pct_cox = cox / (moore + cox + lashar + wallace + harding + sefcik + gov_writein) * 100) |>
  mutate(pct_brown = brown/(brown + peroutka + ag_writein) * 100) |>
  mutate(pct_peroutka = peroutka/(brown + peroutka + ag_writein) * 100) |>
  mutate(dem_diff = pct_moore - pct_brown) |>
  mutate(rep_diff = pct_cox - pct_peroutka)
```
...
md_counties |> inner_join(md_votes_county, by=c('GEOID' = 'geoid'))
...
#Create the map
ggplot() |>
  geom_sf(data=md_votes_county,aes(fill=dem_diff)) |>
  theme_minimal() |>
