...
##### A1: Use NAMELSAD to avoid having a duplicate Baltimore, this way the city and county will be specified.
...
acs_under_5 <- get_acs(geography = 'county', variables = 'B06001_002', year = 2022, state = 'MD')
```
...
##### **A4**: Centrally and the edges of the state.
...
```{r}
# Join the dataframes
md_childcare_income <- md_childcare_pop |>
  left_join(acs_income, 'GEOID')
...
md_childcare_counties |>
  ggplot() +
  geom_point(aes(x=media_income,y=rate)) +
  geom_point(aes(x=media_income,y=rate), method="lm")
...
##### A5: No, there isn't a correlation between income and rate of children per facilities.
