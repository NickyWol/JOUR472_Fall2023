...
**A1.** I'd say the data is somewhat different, but overdose calls are still a bit larger toward the end of the year and in warmer months.

```{r}
pg_calls_percentages <- pg_calls |>
  group_by(month) |>
  summarize(monthly_calls = n()) |>
  mutate(percentage = ((monthly_calls / 1397) * 100))

pg_calls_percentages |> 
  ggplot() +
  geom_bar(aes(x=month, weight=percentage)) +
  labs(
    title="More 911 Overdose Calls in Toward End of Year in PG County",
    x = "Month",
    y = "Pecentage of total calls",
...
**A2.** There is a spike during the summer and the end of the year, which could relate to drug use during warm months and holidays.


```{r}
pg_calls_by_date <- pg_calls |>
  group_by(date) |>
...
```{r}
md_median_income_zip <- get_acs(geography = 'zcta', variable='B19013_001', state='MD'. year=2019)
```
