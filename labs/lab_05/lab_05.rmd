...
```{r}
baltco_911odcalls <- read_csv('data/lab05real.csv', col_names = FALSE)

baltco_911odcalls <- baltco_911odcalls |>
...
A1. The date with the most overdose calls in 2022 was October 4, with a total of 23 calls. 
...
baltco_911odcalls |>
  ...
  summarize(count = n()) |>
  mutate(pct_total = count/3187*100) |>
...
A3. There is one address, 4540 SILVER SPRING RD, that has by far the most calls with 32 while there are a group of others that have around 10 calls.
...
A4. I wonder if there's an association between the days that have the most overdose calls. Whether it's something like the same day of the week, time of year/week or any other similarites between the dates with the most calls.
